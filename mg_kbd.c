...
#define PROMPTL 80
char	 mg_prompt[PROMPTL] = "", *promptp = mg_prompt;
...
getkey(int flag)
{
	...
	if (flag && !pushed) {
		if (mg_prompt[0] != '\0' && ttwait(2000)) {
			/* avoid problems with % */
			ewprintf("%s", mg_prompt);
			/* put the cursor back */
			update(CMODE);
			epresf = KCLEAR;
		}
		if (promptp > mg_prompt)
			*(promptp - 1) = ' ';
	}
	if (pushed) {
		c = pushedc;
	...
	if (flag && promptp < &mg_prompt[PROMPTL - 5]) {
		promptp = getkeyname(promptp,
		    sizeof(mg_prompt) - (promptp - mg_prompt) - 1, c);
		*promptp++ = '-';
		*promptp = '\0';
	}
	return (c);
